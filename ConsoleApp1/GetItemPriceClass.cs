...
public class GetItemPriceClass : IGetItemPrice
{
    ...
    public async Task<SteamItemPrice> GetItemPriceAsync(string appId, string marketHashName)
    {
        try
        {
            // Формируем URL для запроса
            var baseUrl = "https://steamcommunity.com/market/priceoverview/";
            var url = $"{baseUrl}?appid={appId}&market_hash_name={Uri.EscapeDataString(marketHashName)}&currency=1";
